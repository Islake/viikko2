...
const userLocation = {latitude: 60.170833, longitude: 24.9375};

function calculateDistance(restLocation) {
  const latDiff = userLocation.latitude - restLocation.coordinates[1];
  const lonDiff = userLocation.longitude - restLocation.coordinates[0];
...
restaurants.forEach(restaurant => {
  const distance = calculateDistance(restaurant.location).toFixed(2);
  const row = table.insertRow(-1);
  row.insertCell(0).textContent = restaurant.name;
  row.insertCell(
    1
  ).textContent = `${restaurant.address}, ${restaurant.postalCode} ${restaurant.city}`;
  row.insertCell(2).textContent = distance;
});
